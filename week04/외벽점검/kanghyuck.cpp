...
void DFS(int cnt, int index)
{
    ...
    // 자식 노드 호출
    for(int i=index; i<(int)Weak.size(); i++)  // 해결해야할 weak 지점을 순차적으로 확인
    {
        // 친구를 보낼 필요가 없는 경우 (i번째 weak 포인트가 해결된 경우)
        if(map[Weak[i]]==0) continue;  // 취약지점이 해결된 경우
        ...
        for(int j=0; j<(int)Dist.size(); j++)
        {
            ...
            Move(i, Dist[j], 1);  DFS(cnt+1, i+1);  // 시계방향으로 시뮬레이션 후,  (사람수+1, 해결해야할index+1)
            Reset (mapBackup);  // map 복원
            //  2. 반시계방향으로 보내기
            Move(i, Dist[j], 0);  DFS(cnt+1, i+1);  // 반시계방향으로 시뮬레이션 후, (사람수+1, 해결해야할index+1)
