class Solution {
    ...
    public int move(int[] info) {
        ...
        // 아래에서 위
        for (int i = x1; i < x2; i++) {
            this.map[i][y1] = this.map[i + 1][y1]; // 아래 값을 위로 넘겨주기
            // 최소 값 비교
        ...
        // 오른쪽에서 왼쪽
        for (int i = y1; i < y2; i++) {
            this.map[x2][i] = this.map[x2][i + 1]; // 왼쪽 값을 오른쪽으로 넘겨주기
            // 최소 값 비교
        ...
        // 위에서 아래
        for (int i = x2; i > x1; i--) {
            this.map[i][y2] = this.map[i - 1][y2]; // 위 값을 아래로 넘겨주기
            // 최소 값 비교
        ...
        // 왼쪽에서 오른쪽
        for (int i = y2; i > y1; i--) {
            this.map[x1][i] = this.map[x1][i - 1]; // 오른쪽 값을 왼쪽으로 넘겨주기
            // 최소 값 비교
